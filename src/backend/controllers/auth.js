...
Auth.checkLevel = function (clearance) {
    return function(req, res, next) {
        ...
        if (havePermissions) {
            ...
            return res.status(403).send('You do not have sufficient permissions to access.');
        }
    }
}
...
Auth.isLoggedInKey = async function (req, res, next) {
    ...
    if(key){
        await Key.find({key: key}, async function(err, resp){
            if(err){
                //throw err;
                res.status(401).send('Your API key is invalid or has expired.');
                    ...
                    if(err){
                        ...
                        if(resp[0].active==true){
                            await Key.updateOne({_id: resp[0]._id}, {nCalls: resp[0].nCalls+1, lastUsed: Date.now()}, function(err, affected, resp2) {
                                if(err){
                                    res.status(500).send('An error occurred while updating the API Key.');
                                }else{
                                    res.locals.id = resp[0]._id
                                    res.locals.idType = "Key"
                                    //res.locals.idType = "Chave"
                                    next()
                                }
                            })
                        }else{
                            res.status(403).send('Your API key has been disabled, please contact the system administrators.');
                        }
                    }
                });
            }
